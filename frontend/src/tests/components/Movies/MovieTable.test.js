...
describe("MovieTable tests", () => {
  ...
  const expectedHeaders = ["id", "Title", "Rating", "Views"];
  const expectedFields = ["id", "title", "rating", "views"];
  ...
  test("Has the expected column headers, content and buttons", () => {
    ...
    expect(screen.getByTestId(`${testId}-cell-row-0-col-title`)).toHaveTextContent("Spirited Away");

    expect(screen.getByTestId(`${testId}-cell-row-1-col-id`)).toHaveTextContent("3");
    expect(screen.getByTestId(`${testId}-cell-row-1-col-title`)).toHaveTextContent("Monsters, Inc.");
  ...
  test("Edit button navigates to the edit page", async () => {
    ...
    const expectedMessage = `editCallback: {"id":2,"title":"Spirited Away","rating":8.5,"views":100000})`;
  ...
  test("Details button navigates to the details page", async () => {
    ...
    expect(console.log).toHaveBeenCalled();
    const message = console.log.mock.calls[0][0];
    const expectedMessage = `detailsCallback: {"id":2,"title":"Spirited Away","rating":8.5,"views":100000})`;
    expect(message).toMatch(expectedMessage);
    restoreConsole();
  });
  ...
  test("Delete button calls delete callback", async () => {
    ...
    fireEvent.click(deleteButton);
     ...
     const message = console.log.mock.calls[0][0];
     const expectedMessage = `deleteCallback: {"id":2,"title":"Spirited Away","rating":8.5,"views":100000})`;
     expect(message).toMatch(expectedMessage);
     restoreConsole();
  });
});